                           ...
                           # has_cfg = line.split('&')[-1]
                            ...
                            if has_cfg != '-1':  # -1 indicates there exists no cfg for the function
                                if len(edges) >= ParameterConfig.CFG_MIN_EDGE_NUM:
                                    ...
                                    texts.append([[2, 143, 1, 143, 8, 16, 1, 15, 1, 23, 17, 18, 9, 3], [2, 14, 1, 14, 8, 16, 9, 3], [2, 5, 7, 6, 4, 47, 1, 23, 17, 18, 3], [2, 30, 1, 31, 3], [2, 29, 1, 44, 3]])
                                    labels.append(label_maps['#1'])
                                    cfgs.append(['0->1', '1->4', '2->0', '3->2'])
                            indices_set = []
                        continue
                    # 序号化文本，tokenizer句子，并返回每个句子所对应的词语索引
                    indices = []
